    ...
    do
      case $choice in
        ...
        11)
          ...
          sudo su -c 'echo ""                                                >> /OpenWrt/PartOVMF/scripts/1-InstalarPaquetes.sh'
          sudo su -c 'echo "  Reiniciando OpenWrt..."                        >> /OpenWrt/PartOVMF/scripts/1-InstalarPaquetes.sh'
          sudo su -c 'echo ""                                                >> /OpenWrt/PartOVMF/scripts/1-InstalarPaquetes.sh'
        ...
        14)
          ...
          echo ""
          sudo su -c 'echo "#!/bin/sh"                                                                                     > /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          ...
          sudo su -c 'echo "uci set wireless.radio1.channel='auto'"                                                       >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          ...
          sudo su -c 'echo ""                                                                                             >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "# Comprobar si el paquete curl está instalado. Si no lo está, instalarlo."                    >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "  if [ "'"'$'"'"(opkg list-installed | grep -v libcurl | grep curl) = '"''"'  ];then"         >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "    echo '"''"'"                                                                              >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "    echo '"'  curl no está instalado. Iniciando su instalación...'"'"                         >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "    echo '"''"'"                                                                              >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "    opkg update"                                                                              >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "    opkg install curl"                                                                        >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "  fi"                                                                                         >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "curl -s https://raw.githubusercontent.com/nipegun/o-scripts/master/WiFi-Configurar.sh | bash" >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo ""                                                                                             >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
          sudo su -c 'echo "reboot"                                                                                       >> /OpenWrt/PartExt4/root/scripts/4-ActivarWiFi.sh'
